...
<div class="box">
	<div class="columns">
		<div class="column is-two-fifths display-flex content-center align-middle">
			<img
				src="{fileServerUrl}{grandma.pictureUrl}"
				alt="grandma photo"
				class="rounded image is-256x256 center-image object-cover"
			/>
		</div>
		<div class="column">
			<p class="mb-3 flex flex-row items-center">
				<a class="subtitle is-3 text-blue-400 mb-0 mr-3" href="/grandmas/{grandma.username}">
					{grandma.name}
				</a>
				{#if grandma.verified}
					<i class="fas fa-certificate text-yellow-400" />
				{/if}
			</p>
			<p class="icon-text">
				<span class="icon">
				...
				<span class="icon">
					<i class="fas fa-clock text-blue-800" />
				</span>
