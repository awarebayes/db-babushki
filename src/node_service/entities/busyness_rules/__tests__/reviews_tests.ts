...
describe('addReview', () => {
    ...
    const userClaim: UserClaim = { username: 'mock_user', id: '0', expiration: 0};
  
    it('should throw an error if grandma is not found', async () => {
      repos.grandmaRepository.getSingle = jest.fn().mockResolvedValue(null);
      await expect(addReview(repos, userClaim, reviewClaim)).rejects.toEqual("Respected grandma not found!");
    ...
    it('should throw an error if grandma rates herself', async () => {
      ...
      await expect(addReview(repos, newClaim, reviewClaim)).rejects.toEqual("Grandma cannot rate herself!");
      expect(repos.grandmaRepository.getSingle).toHaveBeenCalledWith(0);
    ...
    it('should throw an error if user didnt rate anything', async () => {
      ...
      repos.orderRepository.getOrdersOfUserForGrandma = jest.fn().mockResolvedValue([]);
      await expect(addReview(repos, userClaim, reviewClaim)).rejects.toEqual("User didnt order anything!");
      expect(repos.grandmaRepository.getSingle).toHaveBeenCalledWith(0);
      expect(repos.userRepository.getByUsername).toHaveBeenCalledWith("mock_user");
      expect(repos.orderRepository.getOrdersOfUserForGrandma).toHaveBeenCalledWith(0, 0);
    });
    ...
    it('should succeed if everything was right', async () => {
      ...
      repos.grandmaRepository.getSingle = jest.fn().mockResolvedValue(grandma);
      repos.userRepository.getByUsername = jest.fn().mockResolvedValue(newUser);
      repos.orderRepository.getOrdersOfUserForGrandma = jest.fn().mockResolvedValue([1]);
 ...
 describe('updateReview', () => {
    ...
    it('should throw an error if review is not found', async () => {
      ...
      await expect(updateReview(repos, userClaim, 0, reviewClaim)).rejects.toEqual("Review was not found");
    ...
    it('should throw an error if user tries to update different users review', async () => {
      ...
      repos.reviewRepository.getSingle = jest.fn().mockResolvedValue(otherUsersReview);
      repos.userRepository.getByUsername = jest.fn().mockResolvedValue(userClaim);
      await expect(updateReview(repos, userClaim, 0, reviewClaim)).rejects.toEqual("Trying to update a review of different user!");
      expect(repos.reviewRepository.getSingle).toHaveBeenCalledWith(0);
      expect(repos.userRepository.getByUsername).toHaveBeenCalledWith("mock_user");
    });
 ...
 describe('deleteReview', () => {
    ...
    it('should throw an error if review is not found', async () => {
      repos.reviewRepository.getSingle = jest.fn().mockResolvedValue(null);
      await expect(removeReview(repos, userClaim, 0)).rejects.toEqual("Review was not found");
      expect(repos.reviewRepository.getSingle).toHaveBeenCalledWith(0);
    });
    ...
    it('should throw an error if user tries to delete different users review', async () => {
      ...
      repos.reviewRepository.getSingle = jest.fn().mockResolvedValue(otherUsersReview);
      repos.userRepository.getByUsername = jest.fn().mockResolvedValue(userClaim);
      await expect(removeReview(repos, userClaim, 0)).rejects.toEqual("Trying to delete a review of different user!");
      expect(repos.reviewRepository.getSingle).toHaveBeenCalledWith(0);
      expect(repos.userRepository.getByUsername).toHaveBeenCalledWith("mock_user");
    });
    ...
    it('should succeed if everything was right', async () => {
      repos.reviewRepository.getSingle = jest.fn().mockResolvedValue(review);
      repos.userRepository.getByUsername = jest.fn().mockResolvedValue(userClaim);
      repos.reviewRepository.remove = jest.fn()
      await removeReview(repos, userClaim, 0)
      expect(repos.reviewRepository.getSingle).toHaveBeenCalledWith(0);
      expect(repos.userRepository.getByUsername).toHaveBeenCalledWith("mock_user");
      expect(repos.reviewRepository.remove).toHaveBeenCalledTimes(1);
    });
  });
