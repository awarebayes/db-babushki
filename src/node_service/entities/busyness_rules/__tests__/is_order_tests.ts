...
import { placeOrder, cancelOrder, confirmOrder, startCookingOrder } from "../orders"
...
describe("isOrder", () => {
  ...
  it("should create and cancel successfully", async () => {
    let user = await create_dummy_user()!
    let other_user = await create_dummy_user()!
    expect(user).toBeTruthy()
    ...
    let maybeUser = (await repositories.userRepository.getByUsername(other_claim.username))!
    let maybeOrderToCancel = (await repositories.orderRepository.getSingle(order.id))!
