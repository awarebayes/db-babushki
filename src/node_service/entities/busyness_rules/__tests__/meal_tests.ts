...
describe("getMeals", () => {
  it("should return an array of meals when page is valid", async () => {
    ...
    mockRepositories.mealRepository.getPaged = jest
      .fn()
      .mockResolvedValue(mockMeals);
    const meals = await getMeals(mockRepositories, 1);
    expect(meals).toEqual(mockMeals);
    expect(mockRepositories.mealRepository.getPaged).toHaveBeenCalledWith(
      1,
      25
    );
  });
...
describe("getMealsForGrandma function", () => {
  it("should return an array of meals", async () => {
    ...
    mockRepositories.mealRepository.getMealsOfGrandma = jest
      .fn()
    ...
    expect(
      mockRepositories.mealRepository.getMealsOfGrandma
